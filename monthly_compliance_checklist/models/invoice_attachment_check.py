...
class InvoiceAttachmentCheck(models.Model):
    ...
    def _compute_condition_summary(self):
        ...
        for record in self:
            ...
            # Attachment count
            if (
                record.max_attachments > 0
                and record.min_attachments != record.max_attachments
            ):
                parts.append(
                    f"{record.min_attachments}-{record.max_attachments} attachments"
                )
    ...
    def get_view_ids(cls, env):
        ...
        return {
            "form": env.ref(
                "monthly_compliance_checklist.view_invoice_attachment_check_form"
            ).id,
            "tree": env.ref(
                "monthly_compliance_checklist.view_invoice_attachment_check_tree"
            ).id,
            "search": env.ref(
                "monthly_compliance_checklist.view_invoice_attachment_check_search"
            ).id,
            "action": env.ref(
                "monthly_compliance_checklist.action_invoice_attachment_check"
            ).id,
        }
    ...
    def evaluate_condition(self, year, month):
        ...
        if not matching_moves:
            return False, {
                "message": "No invoices found meeting attachment requirements"
            }
