...
class MonthlyChecklist(models.Model):
    ...
    _sql_constraints = [
        ('unique_period', 'UNIQUE(period_date)', 'A checklist already exists for this period!')
    ]
    ...
    notes = fields.Text(string='Notes', help='Additional notes about this monthly checklist')
    ...
    def _compute_totals(self):
        for record in self:
            ...
            record.completed_items = len(record.check_instances.filtered('is_completed'))
    
    @api.depends('total_items', 'completed_items')
    def _compute_completion_percentage(self):
        for record in self:
            if record.total_items > 0:
                record.completion_percentage = (record.completed_items / record.total_items) * 100
            else:
