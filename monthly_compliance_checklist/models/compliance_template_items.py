...
class ComplianceTemplateItem(models.Model):
    ...
    ], string='Condition Type', required=True, default='multi_condition')
    ...
    move_type = fields.Selection([
        ...
        ('entry', 'Journal Entry'),
        ('any', 'Any Move Type')
    ], string='Move Type', default='any')
    
    # Amount conditions
    minimum_amount = fields.Float(
        string='Minimum Amount',
        help='Minimum transaction amount required'
    )
    maximum_amount = fields.Float(
        string='Maximum Amount', 
        help='Maximum transaction amount allowed'
    )
    ...
    require_reconciliation = fields.Boolean(
        string='Require Reconciliation',
        help='Transaction must be reconciled'
    )
    ...
    def evaluate_condition(self, year, month):
        ...
        if self.transaction_type != 'any':
            if self.transaction_type == 'income':
                domain.append(('line_ids.account_id.account_type', 'in', ['income', 'income_other']))
            elif self.transaction_type == 'expense':
                domain.append(('line_ids.account_id.account_type', 'in', 
