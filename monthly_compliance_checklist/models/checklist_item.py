...
class ChecklistItem(models.Model):
    ...
    def _compute_related_moves(self):
        ...
        for item in self:
            ...
            move_details = [f"{m.name} ({m.partner_id.name if m.partner_id else 'No Partner'})" for m in moves]
            print(f"CONSOLE: Item '{item.name}' found {len(moves)} matching transactions: {move_details}")
            _logger.warning(f"[CHECKLIST DEBUG] Item '{item.name}' found {len(moves)} matching transactions: {move_details}")
