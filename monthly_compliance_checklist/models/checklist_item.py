...
class ChecklistItem(models.Model):
    ...
    checklist_id = fields.Many2one(
        "monthly.checklist", string="Checklist", required=True, ondelete="cascade"
    )

    # Item configuration
    item_type = fields.Selection(
        [("account", "Account Transaction"), ("partner", "Partner Transaction")],
        string="Item Type",
        required=True,
        default="account",
    )
    ...
    required_transaction_type = fields.Selection(
        [("income", "Income"), ("expense", "Expense"), ("any", "Any")],
    ...
    related_moves = fields.Many2many(
        "account.move", string="Related Transactions", compute="_compute_related_moves"
    )
    ...
    def _compute_related_moves(self):
        ...
        for item in self:
            ...
            if item.checklist_id.year and item.checklist_id.month:
                ...
                print(
                    f"CONSOLE: Item '{item.name}' searching for transactions between {start_date} and {end_date}"
                )
                _logger.warning(
                    f"[CHECKLIST DEBUG] Item '{item.name}' searching for transactions between {start_date} and {end_date}"
                )
            ...
            if item.item_type == "account" and item.account_id:
                ...
                print(
                    f"CONSOLE: Item '{item.name}' filtering by account: {item.account_id.name} (ID: {item.account_id.id})"
                )
                _logger.warning(
                    f"[CHECKLIST DEBUG] Item '{item.name}' filtering by account: {item.account_id.name} (ID: {item.account_id.id})"
                )
                ...
                if item.required_transaction_type == "income":
                    ...
                    print(
                        f"CONSOLE: Item '{item.name}' filtering for income transaction types: out_invoice, out_refund"
                    )
                    _logger.warning(
                        f"[CHECKLIST DEBUG] Item '{item.name}' filtering for income transaction types: out_invoice, out_refund"
                    )
                    ...
                    print(
                        f"CONSOLE: Item '{item.name}' filtering for expense transaction types: in_invoice, in_refund, entry"
                    )
                    _logger.warning(
                        f"[CHECKLIST DEBUG] Item '{item.name}' filtering for expense transaction types: in_invoice, in_refund, entry"
                    )
            ...
            # Filter by partner
                ...
                print(
                    f"CONSOLE: Item '{item.name}' filtering by partner: {item.partner_id.name} (ID: {item.partner_id.id})"
                )
                _logger.warning(
                    f"[CHECKLIST DEBUG] Item '{item.name}' filtering by partner: {item.partner_id.name} (ID: {item.partner_id.id})"
                )

            print(f"CONSOLE: Item '{item.name}' final search domain: {domain}")
            _logger.warning(
                f"[CHECKLIST DEBUG] Item '{item.name}' final search domain: {domain}"
            )

            moves = self.env["account.move"].search(domain)
            move_details = [
                f"{m.name} ({m.partner_id.name if m.partner_id else 'No Partner'})"
                for m in moves
            ]
            print(
                f"CONSOLE: Item '{item.name}' found {len(moves)} matching transactions: {move_details}"
            )
            _logger.warning(
                f"[CHECKLIST DEBUG] Item '{item.name}' found {len(moves)} matching transactions: {move_details}"
            )
    ...
    def _compute_debug_info(self):
        ...
        for item in self:
            ...
            debug_lines.append(
                f"Required Transaction Type: {item.required_transaction_type}"
            )
            ...
            if item.partner_id:
                debug_lines.append(
                    f"Partner: {item.partner_id.name} (ID: {item.partner_id.id})"
                )
            if item.account_id:
                debug_lines.append(
                    f"Account: {item.account_id.name} (ID: {item.account_id.id})"
                )
            ...
            if item.item_type == "account" and item.account_id:
                ...
                if item.required_transaction_type == "income":
                    ...
                    debug_lines.append(
                        "Transaction Types: in_invoice, in_refund, entry"
                    )
            elif item.item_type == "partner" and item.partner_id:
                domain.append(("partner_id", "=", item.partner_id.id))
            ...
            try:
                ...
                if moves:
                    for move in moves[:5]:  # Show first 5
                        debug_lines.append(
                            f"  - {move.name} ({move.date}) - {move.partner_id.name if move.partner_id else 'No Partner'}"
                        )
                    if len(moves) > 5:
        ...
        for item in self:
            if item.template_item_id:
                ...
                is_met, details = item.template_item_id.evaluate_condition(
                    item.checklist_id.year, item.checklist_id.month
                )
    ...
    def reevaluate_condition(self):
        ...
        print(
            f"CONSOLE: Partner: {self.partner_id.name if self.partner_id else 'None'} (ID: {self.partner_id.id if self.partner_id else 'None'})"
        )
        print(
            f"CONSOLE: Account: {self.account_id.name if self.account_id else 'None'} (ID: {self.account_id.id if self.account_id else 'None'})"
        )
    ...
    def debug_search_criteria(self):
        ...
        _logger = logging.getLogger(__name__)
        _logger.warning(
            f"[CHECKLIST DEBUG] === DEBUG SEARCH CRITERIA FOR {self.name} ==="
        )
        _logger.warning(f"[CHECKLIST DEBUG] Item type: {self.item_type}")
        _logger.warning(
            f"[CHECKLIST DEBUG] Partner: {self.partner_id.name if self.partner_id else 'None'} (ID: {self.partner_id.id if self.partner_id else 'None'})"
        )
        _logger.warning(
            f"[CHECKLIST DEBUG] Account: {self.account_id.name if self.account_id else 'None'} (ID: {self.account_id.id if self.account_id else 'None'})"
        )
        _logger.warning(
            f"[CHECKLIST DEBUG] Required transaction type: {self.required_transaction_type}"
        )
