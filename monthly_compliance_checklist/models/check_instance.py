                ...
                if template.template_type == 'partner_payment':
                    ...
                    if template.compliance_check_id:
                        ...
                        if hasattr(check_record, 'partner_id'):
                            debug_lines.append(f"Compliance Check Partner: {check_record.partner_id.name if check_record.partner_id else 'None'} (ID: {check_record.partner_id.id if check_record.partner_id else 'None'})")
                        else:
                    ...
                    if instance.year and instance.month:
                        ...
                        try:
                            is_met, details = template.evaluate_condition(instance.year, instance.month)
                            debug_lines.append(f"Evaluation Result: {'PASS' if is_met else 'FAIL'}")
                            debug_lines.append(f"Evaluation Message: {details.get('message', 'No message')}")
                            if 'move_count' in details:
                                debug_lines.append(f"Matching Transactions: {details['move_count']}")
                            if 'total_amount' in details:
                                debug_lines.append(f"Total Amount: {details['total_amount']}")
                            if 'move_ids' in details:
                        ...
                        if template.compliance_check_id and hasattr(template.compliance_check_id, 'partner_id') and template.compliance_check_id.partner_id:
                            ...
                            try:
                                ...
                                debug_lines.append(f"Partner-Filtered Transactions Found: {len(moves)}")
                                if moves:
                                    for move in moves[:5]:
                                        debug_lines.append(f"  - {move.name} ({move.date}) - Amount: {move.amount_total} - Partner: {move.partner_id.name if move.partner_id else 'No Partner'}")
                                    if len(moves) > 5:
