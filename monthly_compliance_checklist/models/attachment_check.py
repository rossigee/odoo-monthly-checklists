...
class AttachmentCheck(models.Model):
    ...
    def get_view_ids(cls, env):
        ...
        return {
            "form": env.ref(
                "monthly_compliance_checklist.view_attachment_check_form"
            ).id,
            "tree": env.ref(
                "monthly_compliance_checklist.view_attachment_check_tree"
            ).id,
            "search": env.ref(
                "monthly_compliance_checklist.view_attachment_check_search"
            ).id,
            "action": env.ref(
                "monthly_compliance_checklist.action_attachment_check"
            ).id,
        }
    ...
    def _check_move_attachments(self, move):
        ...
        if len(attachments) < self.min_attachments:
            return (
                False,
                f"Only {len(attachments)} attachments found, need at least {self.min_attachments}",
            )

        if self.max_attachments > 0 and len(attachments) > self.max_attachments:
            return (
                False,
                f"Too many attachments: {len(attachments)}, maximum allowed: {self.max_attachments}",
            )
        ...
        if self.filename_pattern:
            ...
            if not pattern_matches:
                return (
                    False,
                    f"No attachments match filename pattern: {self.filename_pattern}",
                )
        ...
        if self.min_filesize_kb or self.max_filesize_kb:
            for attachment in attachments:
                ...
                if self.min_filesize_kb and size_kb < self.min_filesize_kb:
                    return (
                        False,
                        f"Attachment {attachment.name} too small: {size_kb:.1f}KB < {self.min_filesize_kb}KB",
                    )
                if self.max_filesize_kb and size_kb > self.max_filesize_kb:
                    return (
                        False,
                        f"Attachment {attachment.name} too large: {size_kb:.1f}KB > {self.max_filesize_kb}KB",
                    )
