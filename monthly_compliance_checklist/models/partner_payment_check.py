...
class PartnerPaymentCheck(models.Model):
    ...
    def _compute_condition_summary(self):
        ...
        for record in self:
            ...
            if record.check_type == "sum":
                ...
                parts = [
                    f"Partner '{record.partner_id.name}' must have at least one payment"
                ]
            ...
            if amount_conditions:
                parts.append(
                    f"with {'total ' if record.check_type == 'sum' else ''}amount {' and '.join(amount_conditions)}"
                )
    ...
    def get_view_ids(cls, env):
        ...
        return {
            "form": env.ref(
                "monthly_compliance_checklist.view_partner_payment_check_form"
            ).id,
            "tree": env.ref(
                "monthly_compliance_checklist.view_partner_payment_check_tree"
            ).id,
            "search": env.ref(
                "monthly_compliance_checklist.view_partner_payment_check_search"
            ).id,
            "action": env.ref(
                "monthly_compliance_checklist.action_partner_payment_check"
            ).id,
        }
    ...
    def evaluate_condition(self, year, month):
        ...
        if not moves:
            return False, {
                "message": f"No transactions found for partner {self.partner_id.name}"
            }
