...
class CheckTemplate(models.Model):
    ...
    def _compute_condition_summary(self):
        ...
        for record in self:
            if record.compliance_check_id and hasattr(record.compliance_check_id, 'condition_summary'):
    ...
    ], string='Expense Category', default='utilities')
    ...
    expected_range_min = fields.Float(
        string='Expected Min Amount',
        help='Minimum expected amount for this expense'
    )
    expected_range_max = fields.Float(
        string='Expected Max Amount',
        help='Maximum expected amount for this expense'
    )
    ...
    supplier_validation = fields.Boolean(
        string='Validate Supplier',
        help='Check against approved vendor list'
    )
    ...
    balance_type = fields.Selection([
        ...
        ('zero', 'Zero Balance'),
        ('range', 'Balance Range')
    ], string='Balance Type', default='range')
    target_balance = fields.Float(
        string='Target Balance',
        help='Expected account balance'
    )
    balance_tolerance = fields.Float(
        string='Balance Tolerance',
        help='Allowed variance from target balance'
    )
    ...
    count_filter_amount_min = fields.Float(
        string='Min Amount Filter',
        help='Count only transactions above this amount'
    )

    # === AMOUNT THRESHOLD CHECK FIELDS ===
    threshold_amount = fields.Float(
        string='Threshold Amount',
        help='Amount threshold to check against'
    )
    threshold_operator = fields.Selection([
        ...
        ('<', 'Less than'),
        ('=', 'Exactly equal')
    ], string='Threshold Operator', default='>=')
    ...
    custom_domain = fields.Text(
        ...
        help='Advanced: Python domain filter as text'
    )
    ...
    def action_create_compliance_check(self):
        ...
        for model_name in self.env.registry:
            ...
            if (hasattr(model_class, '_compliance_check_type') and
                model_class._compliance_check_type == self.template_type):
    ...
    def _evaluate_partner_payment(self, start_date, end_date, instance=None):
        ...
        if instance:
            ...
            available_lines = move_lines.filtered(lambda line: line.id not in matched_line_ids)
        else:
        ...
        selected_line = available_lines.sorted(key=lambda line: line.date, reverse=True)[0]
    ...
    def _evaluate_attachment_check(self, start_date, end_date):
        ...
        if not matching_moves:
            return False, {'message': 'No transactions found meeting attachment requirements'}
    ...
    def _evaluate_recurring_expense(self, start_date, end_date):
        ...
        if not moves:
            return False, {'message': f'No transactions found in expense accounts for {self.expense_category}'}

        # Filter by approved suppliers if validation enabled
        if self.supplier_validation and self.approved_partners:
            approved_moves = moves.filtered(lambda m: m.partner_id in self.approved_partners)
            if not approved_moves:
                return False, {'message': f'No transactions found from approved suppliers for {self.expense_category}'}
        ...
        if self.expected_range_min or self.expected_range_max:
            ...
            if self.expected_range_min and total_amount < self.expected_range_min:
                ...
                return not self.variance_alert, {'message': message, 'move_ids': moves.ids}
    ...
    def _evaluate_amount_threshold(self, start_date, end_date):
        ...
        if self.aggregate_function == 'sum':
            ...
            return False, {'message': f'Unknown aggregate function: {self.aggregate_function}'}
        ...
        if self.threshold_operator == '>=':
            ...
            return False, {'message': f'Unknown threshold operator: {self.threshold_operator}'}
