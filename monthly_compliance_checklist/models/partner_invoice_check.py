...
class PartnerInvoiceCheck(models.Model):
    ...
    require_posted = fields.Boolean(
        string="Must be Posted", default=True, help="Invoice must be in posted state"
    )
    require_payment = fields.Boolean(
        string="Must be Paid", help="Invoice must be fully paid/reconciled"
    )
    ...
    def _compute_condition_summary(self):
        ...
        for record in self:
            ...
            parts = [
                f"Partner '{record.partner_id.name}' must have at least one {invoice_type}"
            ]
    ...
    def get_view_ids(cls, env):
        ...
        return {
            "form": env.ref(
                "monthly_compliance_checklist.view_partner_invoice_check_form"
            ).id,
            "tree": env.ref(
                "monthly_compliance_checklist.view_partner_invoice_check_tree"
            ).id,
            "search": env.ref(
                "monthly_compliance_checklist.view_partner_invoice_check_search"
            ).id,
            "action": env.ref(
                "monthly_compliance_checklist.action_partner_invoice_check"
            ).id,
        }
    ...
    def evaluate_condition(self, year, month):
        ...
        if not moves:
            return False, {
                "message": f"No invoices found for partner {self.partner_id.name}"
            }
